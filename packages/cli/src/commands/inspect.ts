...
import type { ExpandedData, SchemaModel, InspectResult, ProofArtifact } from '@mimicai/core';
...
export function registerInspectCommand(program: Command): void {
  const inspect = program
    .command('inspect')
    .description('Show schema, data, or proof information');
  ...
  // ── mimic inspect proofs ────────────────────────────────────────────────
  inspect
    .command('proofs')
    .description('List V5 proof artifacts from .mimic/proof/')
    .option('--verbose', 'enable verbose logging')
    .action(async (opts) => {
      await inspectProofs(opts);
    });
...
async function inspectProofs(opts: ProofOptions): Promise<void> {
  if (opts.verbose) logger.setVerbose(true);

  const cwd = process.cwd();
  await loadConfig(cwd);

  logger.header('mimic inspect proofs');

  const proofDir = join(cwd, '.mimic', 'proof');
  if (!(await fileExists(proofDir))) {
    logger.warn('No proof directory found. Run "mimic run" first.');
    return;
  }

  let files: string[];
  try {
    files = (await readdir(proofDir)).filter((f) => f.endsWith('.json'));
  } catch {
    files = [];
  }

  if (files.length === 0) {
    logger.info('No proof artifacts found.');
    return;
  }
  ...
  for (const file of files.sort()) {
    try {
      const artifact = await readJson<ProofArtifact>(join(proofDir, file));
      table.push([
        chalk.yellow(artifact.runId),
        chalk.dim(String(artifact.proofVersion)),
        chalk.yellow(String(artifact.records.length)),
        chalk.dim(artifact.emittedAt),
      ]);
    } catch {
      table.push([chalk.yellow(file.replace('.json', '')), chalk.red('(parse error)'), '', '']);
    }
  }
