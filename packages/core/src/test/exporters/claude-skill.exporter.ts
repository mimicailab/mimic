 ...
 * `<outputDir>/skills/mimic-eval/SKILL.md` that Claude Code picks up
 * automatically when the file is also placed under the project's `skills/` dir.
 *
 * The skill body instructs Claude Code to load `mimic-scenarios.json`, fan out
 * one sub-agent per scenario, score with hybrid (strict + LLM-judge paraphrase
 * + numeric range) checks, and print a markdown report.
 *
 * The MCP server list is derived from the union of `metadata.platform` across
 * the supplied scenarios — no extra config required.
 */
export interface ClaudeSkillExporterOptions {
  /**
   * Name of the target agent skill the eval will exercise (e.g. `briefing-prep`).
   * If omitted, the SKILL.md will use a `<your-target-skill>` placeholder.
   */
  targetSkill?: string;
}

const DEFAULT_TARGET_SKILL_PLACEHOLDER = '<your-target-skill>';
...
export class ClaudeSkillExporter implements ScenarioExporter {
  ...
  async export(
    ...
    const target = this.options.targetSkill ?? DEFAULT_TARGET_SKILL_PLACEHOLDER;
    const persona = scenarios[0]?.metadata.persona ?? 'default';
    const mcpServers = derivePlatformList(scenarios);
...
function derivePlatformList(scenarios: MimicScenario[]): string[] {
  const set = new Set<string>();
  for (const s of scenarios) {
    if (s.metadata.platform) set.add(s.metadata.platform);
  }
  return [...set].sort();
}
...
Persona under test: \`${persona}\`. Scenarios at generation time: ${scenarioCount}.
...
- **Target skill** — default \`${targetSkill}\`. Override only if the user names a different one.
...
\`\`\`
You are the agent under test in a Mimic eval. Produce a response exactly as the \`${targetSkill}\` skill prescribes. The skill markdown lives at \`skills/${targetSkill}/SKILL.md\` (relative to repo root); read it first to learn the steps and the output format.

You have access to MCP tools for ${mcpLine} — fan out across them per the skill's instructions.

User request:
<scenario.input>

Output ONLY the rendered response text (no preamble, no explanation, no tool-call narration). The eval grader will read the response verbatim.
\`\`\`
...
For each scenario that has at least one \`candidate-miss\` substring (skip those with all-strict-pass to save cost), reason inline:
...
- **The target skill is configurable** — if the user says "eval against the <other> skill" or "use my custom v2 skill", swap the path in the sub-agent prompt template accordingly. Default is \`${targetSkill}\`.
